...
![竹知了 Blender 循环动画 · 侧向视角](zhuzhiliao-loop-b.webp)
...
![弹性绳加速度节点组](node-elastic-rope.png)
...
![Geometry Nodes 修改器控制面板](controls-panel.png)

所有主要控制都集中在 `物理竹知了` 对象的 `绳物理参数 · Simulation Zone` 修改器中：

| 参数 | 默认值 | 调整后会发生什么 |
|---|---:|---|
| 绳长 | 0.79 m | 决定知了距离杆端的活动半径；越长，摆动轨迹越大。 |
| 重力 | 0.08 | 越大越容易下坠，摆动恢复方向也越明显。 |
| 刚度 | 55 | 越大越接近硬绳，越小则拉伸感更强。 |
| 阻尼 | 0.30 | 消耗速度；调高会更快停下，调低会摆得更久。 |
| 线粗 | 0.0065 m | 只影响可见绳子的粗细，不改变求解结果。 |
| 弹性 | 1.0 | 整体缩放弹性绳的回拉强度。 |
| 知了大小 | 1.0 | 缩放显示模型，便于复用到不同场景。 |
| 锁定绳长 | 关闭 | 开启后强制保持固定半径；关闭时允许绳子松弛和拉伸。 |
| 启用扇动 | 开启 | 独立打开或关闭翅膀动画。 |
| 扇动速度 | 7.0 | 控制每秒扇动节奏。 |
| 扇动幅度 | 20° | 控制翅膀摆角大小。 |
| 翅膀静止角 | 9.7° | 关闭扇动时的基础张开角度。 |

声音参数位于 `声音触发控制器`：`启用声音`、`音量`、`移动阈值`、`静止复位秒数` 和 `音效路径`。声音监听的是摇杆真实变换，因此手动拖动和播放关键帧都能触发。

### 推荐手感

- **传统甩动感**：锁定绳长开启，刚度 55，阻尼 0.25–0.35。
- **软弹力绳**：锁定绳长关闭，刚度 20–35，弹性 0.6–0.9。
- **夸张卡通效果**：重力 0.04，阻尼 0.12，扇动幅度 30°。
- **展示静帧**：重力与扇动关闭，直接旋转甩杆和知了确定构图。
...
├── node-elastic-rope.png           # 弹性绳节点结构截图
├── controls-panel.png              # 修改器参数截图
